...
\vspace*{0.3cm} 
\begin{center} 
	{\Large \bf ML Lecture Notes: Wk 2\\ Training, Divergence, Loss \& Optimisation}
	\vspace{2mm}
...
\begin{tcolorbox}
    ...
    We can get unbasied estimated as $\hat{\epsilon}^2 = (y-\hat{y)^2}$.\\
    
    We assume: $E[\epsilon$ = E[\text{diag}(\hat{\epsilon^2)}$]

    Giving us:

    $$\text{Var}(\hat{\beta}) = (X^TX)^{-1}X^T \textcolor{red}{diag(\hat{e}^2)}X(X^TX)^{-1}$$
...
\begin{tcolorbox}
    Since \( p(D) \) is constant with respect to the parameters \( θ \), it does not affect the argmax operation when we seek to maximize the posterior. \\
...
= maximizing the sum of the log-likelihood (\( \log p(D|θ) \)) and the log-prior (\( \log p(θ) \)). \\
