...
\vspace*{0.3cm} 
\begin{center} 
	{\Large \bf ML Lecture Notes: Wk 7_2\\ Tree-based methods}
	\vspace{2mm}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \usepackage{Standardization doesn't matter}
    \end{itemize}
...
There’s no efficient way to construct the best tree (computationally intractable) $\rightarrown$ in practice, we greedily recurse down the tree.
\begin{itemize}
...
\begin{align*}
    \mathcal{L}(\theta) &= \sum_i^n \el (y_i, f(x_i)) \\
    &= \sum_j^J \sum_{x_i \in R_j} \el (y_i, \hat{y}(R_j))
\end{align*}
