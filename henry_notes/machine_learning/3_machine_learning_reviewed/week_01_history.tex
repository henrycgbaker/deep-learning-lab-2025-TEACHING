...
\chapter{Week 1: Intellectual History of AI and Machine Learning}
...
\textbf{Limitations}: The model was highly idealised. Real neurons have continuous-valued outputs, complex temporal dynamics, and learning capabilities not captured by fixed weights. But as a \emph{proof of concept} that neural-like systems could perform computation, it was profoundly influential.
\end{greybox}
...
\begin{greybox}[Hebbian Learning Rule]
...
\textbf{Problem}: Pure Hebbian learning has no mechanism for weights to decrease, leading to unbounded growth. Modern variants (Oja's rule, BCM theory) incorporate normalisation or competition to address this.
\end{greybox}
...
\begin{itemize}
    ...
    \item \textbf{1948}: Wiener's \emph{Cybernetics} published
...
\begin{itemize}
    \item The difficulty of common-sense reasoning (Moravec's paradox: hard problems are easy, easy problems are hard)
...
\hat{y} = \sign(w^\top x + b)
\]
where $w \in \mathbb{R}^d$ is the weight vector, $b \in \mathbb{R}$ is the bias, and $\sign(z) = +1$ if $z \geq 0$, else $-1$.
...
\textbf{Perceptron Convergence Theorem}: If the training data is \emph{linearly separable} (there exists a hyperplane perfectly separating positive from negative examples), the perceptron algorithm converges in a finite number of steps. The number of mistakes is bounded by $(R/\gamma)^2$ where $R$ is the radius of the data and $\gamma$ is the margin of the best separating hyperplane.

\textbf{Intuition}: Each update rotates the weight vector toward correctly classifying the current mistake. If a perfect separator exists, we eventually find it.
\end{greybox}
...
Notable examples:
\begin{itemize}
    \item \textbf{MYCIN} (Stanford, 1970s): Diagnosed bacterial infections and recommended antibiotics. Used certainty factors to handle uncertainty. Performed comparably to human experts in blind tests, but was never deployed clinically due to liability concerns.
...
\begin{itemize}
    \item \textbf{Forward chaining} (data-driven): Start from known facts, apply rules to derive new facts, continue until the goal is reached or no more rules apply
    \item \textbf{Backward chaining} (goal-driven): Start from the goal, find rules whose conclusions match, then try to establish those rules' premises (recursively)
\end{itemize}
...
\begin{itemize}
    \item \textbf{Self-Organising Maps} (Kohonen, 1982): Neural networks that learn topological maps of high-dimensional data, preserving neighbourhood relationships
    \item \textbf{Hopfield Networks} (1982): Recurrent networks that function as associative (content-addressable) memory, using an energy function framework from statistical physics
    \item \textbf{Boltzmann Machines} (Hinton \& Sejnowski, 1983): Stochastic neural networks that learn probability distributions over their inputs, providing a principled probabilistic framework for neural computation
\end{itemize}
...
\begin{itemize}
    \item \textbf{Temporal Difference Learning} (Sutton, 1988): Learning value functions from experience without waiting for episode completion. TD learning bootstraps-it updates estimates based on other estimates.
    \item \textbf{Q-Learning} (Watkins, 1989): A model-free algorithm for learning optimal policies without knowing transition probabilities. Learns the Q-function $Q(s,a) = R(s,a) + \gamma \sum_{s'} P(s'|s,a) V^*(s')$.
    \item \textbf{TD-Gammon} (Tesauro, 1992): An RL system that achieved expert-level backgammon play through self-play, learning entirely from the game outcomes. A landmark demonstration of RL's potential.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{2022}: ChatGPT brings LLMs to mainstream attention
...
\begin{enumerate}
    \item \textbf{Compute}: GPUs provided massive parallelism for matrix operations. NVIDIA's CUDA (2007) made GPU programming accessible. Training that would take months on CPUs took days on GPUs. The computational performance gains for matrix multiplication were transformative.
    \item \textbf{Data}: The internet generated unprecedented quantities of labelled and unlabelled data. ImageNet (14M images, 1000 classes) provided a challenging benchmark. Social media, digitised text, and web scraping created training sets at previously impossible scales.
    \item \textbf{Algorithms}: Better architectures and training techniques solved old problems:
    \begin{itemize}
        \item ReLU activations (instead of sigmoid) reduced vanishing gradients
        \item Dropout provided effective regularisation
        \item Batch normalisation stabilised training
        \item Residual connections enabled very deep networks
        \item Adam and other adaptive optimisers improved convergence
    \end{itemize}
...
The Transformer architecture (Vaswani et al., 2017, ``Attention Is All You Need'') revolutionised sequence modelling. Previous approaches (RNNs, LSTMs) processed sequences step-by-step, creating bottlenecks. Transformers process all positions in parallel through the \textbf{attention mechanism}-allowing models to dynamically focus on relevant parts of the input regardless of distance.
...
\textbf{Interpretation}: $QK^\top$ computes similarity scores between each query and all keys. Dividing by $\sqrt{d_k}$ prevents scores from becoming too large (which would make softmax saturate). The softmax converts scores to attention weights (probabilities). Finally, we take a weighted sum of values.

\textbf{Multi-head attention} runs several attention operations in parallel with different learned projections, allowing the model to attend to different types of relationships simultaneously.
\end{greybox}
...
\begin{itemize}
    ...
    \item \textbf{Brittleness to distribution shift}: Performance degrades on data unlike training distribution
