...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
    \item \textbf{Conditional independence does not imply independence}\ldots{}A lot of applied causal inference is built on finding conditional independence where there is not independence, e.g. by applying statistical controls. E.g., no selection into treatment given some conditions.
\end{itemize}
\par\noindent\rule{\textwidth}{0.4pt}
\vskip 0.4cm
...
\textit{Even if X and Y are not independent\\ 
    BUT this only works when functions are linear.\ldots{} p.12} \\
