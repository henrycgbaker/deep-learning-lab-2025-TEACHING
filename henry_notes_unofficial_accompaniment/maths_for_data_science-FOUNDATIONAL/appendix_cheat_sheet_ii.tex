...
\label{chap:cheatsheet2}

\section{Week 6: Continuous R.V.s Meets Probability}

\subsection{Continuous r.v.s}
\begin{itemize}
    \item r.v.\ has continuous distribution if its CDF is differentiable
    ...
    \item unlike discrete r.v.s, for continuous: $P(X=x)=0$ for all $x$ \begin{itemize}
        ...
        \item BUT, CDF remains interpretable as $P(X \leq x)$ (as it represents AUC under PDF)
...
\section{Week 7: Continuous R.V.s II}
...
\begin{itemize}
    ...
    \item independence: covariance is 0: $E[XY] = E[X]E[Y]$
\end{itemize}
...
\begin{enumerate}
    ...
    \item $\Cov(aX, Y) = a \cdot \Cov(X, Y)$ for any constant $a$
    \item $\Cov(X + Y, Z) = \Cov(X, Z) + \Cov(Y, Z)$
    \item $\Var(X + Y) = \Var(X) + \Var(Y) + 2 \cdot \Cov(X, Y)$
    \item \textbf{additional:} $E[E[X]]$: expectation of a constant is just a constant.
\end{enumerate}
...
\Cov(X, Y) &= E[XY] - E[X]E[Y] = 5.25 - (1.5 \times 3.5) = 0
\end{align*}
Since the covariance is 0, it suggests that $X$ and $Y$ are uncorrelated
\end{tcolorbox}

\subsection{Law of Large Numbers}
As $n$ grows large, the sample mean $\bar{X}$ converges to the true mean $\mu$:
\begin{itemize}
...
for uniform: \[\Var(X) = \frac{(b - a)^2}{12}\]
...
\begin{enumerate}
    \item Initialise parameters\begin{itemize}
        ...
        \item $\pi_1, \pi_2$ = mixing proportions (the initial prob of being in one distribution)
    \end{itemize}
...
b_3 \\
\end{bmatrix}
= a_1b_1 + a_2b_2 + a_3b_3$ \ldots{} NB: output is scalar
    \item Matrix multiplication
    \begin{itemize}
        \item conformable: $m \times n$ and $n \times p$ (LHS columns = RHS rows) $\rightarrow$ output $m \times p$ - the outer values
    ...
    \item Transpose Facts
        \begin{itemize}
            ...
            \item $(A + B)^T = A^T + B^T$
            \item $(AB)^T = B^T A^T$
            \item $\mathbf{a}^T\mathbf{b} = \mathbf{b}^T\mathbf{a}$
        \end{itemize}
    \item Identity matrix
    \begin{itemize}
        \item $I_n \mathbf{x}_n = \mathbf{x}_n$
        \item $I_m A_{m\times n} = A_{m\times n}$ and $A_{m\times n}I_n = A_{m\times n}$
        \item $A^{-1}A = I_n$
    \end{itemize}
    \item Vector Norms (measure of magnitude)
     \begin{itemize}
        \item L1: $\|\mathbf{x}\|_1 = \sum_{i} |x_i|$ = take absolute values of elements before summing
        \item L2: $\|\mathbf{x}\|_2 = \sqrt{\sum_{i} x_i^2}$ = square root of the sum of the squares of the vector's elements
    \end{itemize}
\end{itemize}
...
This expanded \textbf{cost function} (expressed in matrix notation) is what we will be minimising (by taking first derivative, set to zero)

(1) Setting to 0 $\rightarrow$ (2) taking first derivative with respect to $\boldsymbol{\beta}$ $\rightarrow$ (3) simplifying:
...
\[
\text{Minimize} \left( \text{Residual Sum of Squares} + \lambda \sum_{i=1}^{p} \left| \beta_i \right| \right)
\]
...
\section{Week 9: Linear Algebra II}
\begin{itemize}
    \item Linear Dependence: $c_1\mathbf{v}_1 + c_2\mathbf{v}_2 + \cdots + c_n\mathbf{v}_n = \mathbf{0}$
    \begin{itemize}
        \item 1) set up the equation $c_1\mathbf{v}_1 + c_2\mathbf{v}_2 + \cdots + c_n\mathbf{v}_n = \mathbf{0}$
        \item 2) solve the system of equations: take one $c_n\mathbf{v}_n$ over to the other side so you can write one of the $c_n\mathbf{v}_n$ as a function of the others, then plug back in, etc.\\
        I STILL DONT GET HOW TO SOLVE THESE SYSTEMS?
    \end{itemize}
    \item Span = all linear combinations of the vectors in the set.
    \begin{itemize}
        \item $S$ is a spanning set for $V$ if all the dimensions of $V$ can be represented by linear combinations of $S$. A Spanning Set $S$ must contain at least as many elements as the linearly independent vectors from $V$.
    ...
    \item eigenvalue-vector pairings: \begin{itemize}
        \item $A\mathbf{v} = \lambda \mathbf{v}$
        \item eigenvalue equation: $(\lambda I_n - A) \mathbf{v} = \mathbf{0}$
        \item characteristic equation: $\det(A - \lambda I) = 0$ or $\det(\lambda I_n - A) = 0$ finds eigenvalues
        ...
        \item eigenspace of $\lambda$, $E_\lambda$, is all vectors $\mathbf{v}$ that satisfy the condition $A\mathbf{v} = \lambda \mathbf{v}$
    \end{itemize}
    \item Eigendecomposition: $A = Q\Lambda Q^{-1}$
    \begin{itemize}
        \item $A$ = square matrix $n \times n$
        \item $\Lambda$ = diagonal matrix with eigenvalues of $A$, $n \times n$
    ...
    \begin{itemize}
         ...
         \item $U$ is a matrix containing left singular vectors of $A$ (contains the $m$ eigenvectors of the square matrix $AA^T$) ($m \times m$)
...
\subsection{As Variance Maximisation}

\[\mathbf{z}_1 = \phi_{11}x_1 + \phi_{21}x_2 + \ldots + \phi_{p1}x_p\]
...
\begin{align*}
    \max_{\phi_{11}, \phi_{21}, \ldots, \phi_{p1}} \Var(\mathbf{z}_1) &= \max_{\phi_{11}, \phi_{21}, \ldots, \phi_{p1}} \frac{1}{n} \sum_{i=1}^{n} (z_{i1} - \bar{\mathbf{z}}_1)^2 \\
...
\begin{itemize}
    ...
    \item Eigendecompose that: $S\boldsymbol{\phi}_1 = \lambda_1\boldsymbol{\phi}_1$
    \item $\lambda_1$ = eigenvalue for first principal component
    \item $\boldsymbol{\phi}_1$ = loadings for first principal component
    \item i think you would then multiply the original data matrix by the loadings vector to get the first principal component.
\end{itemize}


\section{Week 10: Optimisation}
\begin{itemize}
    \item 2nd deriv test: pos = min; neg = max
    \item Lagrangian: $\mathcal{L}(x, y, \lambda) = f(x, y) + \lambda g(x, y)$
    \item constrained opti: 1) set up Lagrangian, 2) Take the first derivatives $\frac{\partial \mathcal{L}}{\partial x}$, $\frac{\partial \mathcal{L}}{\partial y}$, and $\frac{\partial \mathcal{L}}{\partial \lambda}$ and set them equal to 0, 3) solve\\
    PRACTICE SOLVING
    \item Hessian: each element a 2nd order partial derivative of a function. Has same dimensions as number of variables in the function: \begin{itemize}
